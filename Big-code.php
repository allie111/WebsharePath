<?php 
    //the webshare function get the user's input and anylise the input to get the path from $ap_sessionsvr_array
    //$ap_sessionsvr_array['DB127']['wsshare']					= "\\\\prodsan-okc-h01-cltst.paycomhq.com\\QAWSSH$\\OKC\\DB127\\";
    //$ap_sessionsvr_array['DB129']['clientstr']					= "0PP00";
    // $ap_sessionsvr_array['DB129']['clientend']					= "0PT99";
    
    $input = readline('Please enter the Client Code (5-digit):');

    check_input($input);
    
    //check user's input if it is 5 digits for client code
    function check_input($cc_code){
        if($cc_code){
            //split the input and convert to array
            $client_code = preg_split('//', $cc_code, -1, PREG_SPLIT_NO_EMPTY);
            //check if the array has 5 elements.
            if(sizeof($client_code)== 5){
                echo "True";
                webshare($client_code);
            }
        else{
            echo " User input invalid.";
            $input = readline('Please enter the Client Code:');
            check_input($input);
            }
        }
        
        
    }

    

    function webshare(array $input_cc){
        
        // $DBlevel10_35 = array('A' => '10', 'B' => '11', 'C' => '12', 'D' => '13', 'E' => '14', 'F' => '15', 'G' => '16', 'H' => '17',
        //  'I' => '18', 'J' => '19', 'K' => '20', 'L' => '21', 'M' => '22', 'N' => '23', 'O' => '24', 'P' => '25', 'Q' => '26',
        //   'R' => '27', 'S' => '28', 'T' => '29', 'U' => '30', 'V' => '31', 'W' => '32', 'X' => '33', 'Y' => '34', 'Z' => '35');
        
        $DBlevel10_35 = array('A' => 'DB10', 'B' => 'DB11', 'C' => 'DB12', 'D' => 'DB13', 'E' => 'DB14', 'F' => 'DB15', 'G' => 'DB16', 'H' => 'DB17',
         'I' => 'DB18', 'J' => 'DB19', 'K' => 'DB20', 'L' => 'DB21', 'M' => 'DB22', 'N' => 'DB23', 'O' => 'DB24', 'P' => 'DB25', 'Q' => 'DB26',
          'DBR' => '27', 'S' => 'DB28', 'T' => 'DB29', 'U' => 'DB30', 'V' => 'DB31', 'W' => 'DB32', 'X' => 'DB33', 'Y' => 'DB34', 'Z' => 'DB35');
        
        if($input_cc){
            //include the array file to get the wspath
            include 'sample.php';
            //print_r($input_cc);

            

            if($input_cc[0] == '0'){
                echo " it is 0 " .$input_cc;
                //check the second digit
                //if 2nd digit contains only number 

                // check if all the elements in the array are numerical.
                if(array_map("is_numeric", $client_code)){

                    //echo " TRUE NUMBER!";
                    print_r( $client_code);
                    $key_level = "DB".$client_code[0].$client_code[1];
                    echo $key_level;
                    echo gettype($key_level);
                    if(array_key_exists($key_level, $array_sample)){
                        echo "get the value WOOHOO from 0 - 9999" . $array_sample[$key_level]["webshare"];
                    }
                        
                }
                

                // ************
                //if 2nd digit contains the letter and 3rd digit does not contain the letter, 4-5 are numbers
                elseif(('A' <= $input_cc[1] && $input_cc[1] <='Z') && !('A' <= $input_cc[2] && $input_cc[2] <='Z') && (0 <= (int)$input_cc[3] 
                && (int)$input_cc[3] <=9)&& (0 <= (int)$input_cc[4] && (int)$input_cc[4] <=9)){
                    
                    //check if the sencond digit is in the key of DB level 10-25 array
                    if(array_key_exists($input_cc[1], $DBlevel10_35)){
                        //echo $DBlevel10_35[$input_cc]; //output the level
                        //find the path through given level
                        $arrayKeys = array_keys($array_sample);
                        if(array_key_exists($input_cc[1], $DBlevel10_35)){
                            $level_number = $DBlevel10_35[$input_cc[1]]; //output the level
                           // $match_level = str_contains($level_number, $arrayKeys);
                           $matching_key = array_key_exists($level_number, $arrayKeys);
                           echo "get the value WOOHOO" . $array_sample[$level_number]["webshare"];
                        }
                        
                    }
                }
                //check 2nd and 3rd digits contain letters '0AB00'
                elseif(('A' <= $input_cc[1] && $input_cc[1] <='Z') && ('A' <= $input_cc[2] && $input_cc[2] <='Z')  
                && (0 <= (int)$input_cc[3] && (int)$input_cc[3] <=9)&& (0 <= (int)$input_cc[4] && (int)$input_cc[4] <=9)){
                    //2nd in range from A-Z and 3rd digit in range A-E
                    echo "@@@@ 2 letters";
                    
            }


            elseif($client_code[0] == '1'){
                echo " it is 1 " .$input_cc;
            }
            else{
                $input = readline('Please enter the Client Code:');
                webshare($input_cc);
            }
    
        }
        // else{
        //     return 0;
        // }
    
    
    }
    //function check the client code with 2 letters

    function two_letters(array $client2letters, array $webshare_2letters){
        $DBlevel36_61 = array('A' => 'DB36', 'B' => 'DB37', 'C' => 'DB12', 'D' => 'DB13', 'E' => 'DB14', 'F' => 'DB15', 'G' => 'DB16', 'H' => 'DB17',
         'I' => 'DB18', 'J' => 'DB19', 'K' => 'DB20', 'L' => 'DB21', 'M' => 'DB22', 'N' => 'DB23', 'O' => 'DB24', 'P' => 'DB25', 'Q' => 'DB26',
          'DBR' => '27', 'S' => 'DB28', 'T' => 'DB29', 'U' => 'DB30', 'V' => 'DB31', 'W' => 'DB32', 'X' => 'DB33', 'Y' => 'DB34', 'Z' => 'DB35');
        if('A' >= $client2letters[1] && $client2letters[1] <='Z'){
            if('A' >= $client2letters[2] && $client2letters[2] <='E'){

        }
    }  
    
}
   
}   
    
    
            
?> 
